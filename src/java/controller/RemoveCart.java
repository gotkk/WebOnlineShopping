...
public class RemoveCart extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Shoppingcart> newcartLists = new ArrayList<Shoppingcart>();
        Iterator<Shoppingcart> itr = cartList.iterator();
        Shoppingcart spc = null;
        while (itr.hasNext()) {
            spc = itr.next();
            if (spc.getId() != (Integer.parseInt(request.getParameter("removeID")))) {
                newcartLists.add(spc);
            }
        }
        session.setAttribute("cartLists", newcartLists);
